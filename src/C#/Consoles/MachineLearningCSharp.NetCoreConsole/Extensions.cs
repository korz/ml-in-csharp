...
namespace MachineLearningCSharp.NetCoreConsole
{
    public static class Extensions
    {
        public static IReadOnlyCollection<NamedOnnxValue> CreateTensor2(this InferenceSession session, float[] vector, int?[] dimensionShape = null)
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                var name = session.InputMetadata.Keys.ElementAt(i);
                tensorValues.Add(NamedOnnxValue.CreateFromTensor(name, new DenseTensor<float>(new Memory<float>(vector,i,2), dimensions)));
            }
        ...
        {
            var tensor = session.CreateTensor(new float[] { p, q }, new int?[]{2,2,1});
